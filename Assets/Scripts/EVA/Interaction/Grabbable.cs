...
namespace EVA.Interaction {
    public class Grabbable : PointableElement, IGrabbable {
        ...
        [Optional]
        private MonoBehaviour _oneGrabTransformer;

        [SerializeField]
        [Interface(typeof(ITransformer))]
        [Optional]
        private MonoBehaviour _twoGrabTransformer;
        ...
        public List<Pose> GrabPoints => _selectingPoints.Select(
            (p, i) => (i, p))
            .Where(t => GameplayManager.Instance.GrabInteractors.Select(gi => gi.Identifier)
                .Contains(_selectingPointIds[t.i]))
            .Select(t => t.p)
            .ToList();
        ...
        private void BeginTransform() {
            ...
            int useGrabPoints = GrabPoints.Count;
            if (_maxGrabPoints != -1) {
